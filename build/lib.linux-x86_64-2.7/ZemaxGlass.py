...
class ZemaxGlassLibrary(object):
    ...
    def __init__(self, dir = None, wavemin = 400.0, wavemax = 700.0, nwaves = 300, catalog = 'all', sampling_domain = 'wavelength',
        ...
        self.pressure_dispersion_measurement_reference = 0.10113e6  # Pa
    ...
    ## =========================
    def get_dispersion(self, glass, catalog, temperature_environment = 20., pressure_environment = 0.10113e6, force_update = False):
        '''
        ...
        temperature = 20 Degree Celsius: float
            The temperature of the environment, e.g. air.
        pressure_environment = 0.10113e6 Pa : float
            The pressure of the enviornment, e.g. air at normal conditions. For vacuum set this value to zero.
        ...
        if ('indices' in self.library[catalog][glass] and not force_update):
            return(self.waves, self.library[catalog][glass]['indices'])

        if (catalog == None):
            print('Warning: cannot find glass "' + glass + '" in the library! Aborting ...')
            return(None, None)
        if ('waves' in self.library[catalog][glass]) and ('indices' in self.library[catalog][glass] and not force_update):
            return(self.waves, self.library[catalog][glass]['indices'])
        ...
        # Include pressure and temperature dependence of Environemnt.
        # From Applied Optics: TIE-19: Temperature Coefficient of the Refractive Index
        td = self.library[catalog][glass]['td']
        temperature_dispersion_measurement_reference = td[6]  # Degree Celsius

        p_ref = self.pressure_dispersion_measurement_reference
        T_ref = temperature_dispersion_measurement_reference
        T = temperature_environment
        p = pressure_environment

        n_ref = 1 + (6432.8 + ((2949810. * w ** 2) / (146.*w ** 2 - 1)) + ((25540 * w ** 2) / (41 * w ** 2 - 1))) * 1e-8

        n_air_ref = 1. + ((n_ref - 1.) / (1. + (T_ref - 15) * 3.4785e-3)) * p_ref / p_ref

        n_environment = 1. + ((n_ref - 1.) / (1. + (T - 15) * 3.4785e-3)) * p / p_ref

        indices = indices * n_air_ref / n_environment
    ...
    ## =========================
    def get_temperature_dependence(self, glass, catalog, temperatures, temperature_environment = 20, pressure_environment = 0.10113e6):
        '''
        For a given glass, calculate the temperature dependence of the refractive index according to the equation in the ZEMAX manual
        ...
        catalog : str
            The catalog containing the glass.
        temperatures : ndarray [Kelvin]
            A numpy array containing the temperatures at which the refractive index shall be calculated

        Returns
        -------
        indices : ndarray
            A numpy array with the temperature dependence of the glass at the following wavelengths
        '''
        ...
        wavelength, index_of_refraction = self.get_dispersion(glass, catalog, temperature_environment, pressure_environment)
        wavelength = wavelength / 1000.
        n = index_of_refraction
        delta_T = temperatures - 273.15 - td[-1]

        delta_n_abs = einsum('i,ij->ij', (n ** 2 - 1) / (2 * n), (td[0] * delta_T + td[1] * delta_T ** 2 + td[2] * delta_T ** 3 + \
                                            einsum('j,i->ij', (td[3] * delta_T + td[4] * delta_T ** 2), 1 / (wavelength ** 2 - sign(td[5]) * td[5] ** 2))))

        index_of_refraction = index_of_refraction + delta_n_abs.T

        return wavelength, temperatures, index_of_refraction
    ...
    ## =========================
    def plot_temperature_dependence(self, glass, catalog, temperatures):
        '''
        Plot the glass refractive index curve as a function of temeprature for given wavelengths.
        ...
        catalog : str
            The catalog containing the glass.
        temepratures : ndarray
            Array containing the values for which the refractive indes shall be plotted
        '''

        (wavelengths, temperatures, index_of_refraction) = self.get_temperature_dependence(glass, catalog, temperatures)
        fig = plt.figure(figsize = (10, 5))
        ax = fig.add_subplot(111)
        ax.plot(temperatures, index_of_refraction, 'b-', linewidth = 2)

        plt.title(glass + ' temperature dependence')
...
def read_library(glassdir, catalog = 'all'):
    ...
    for f in files:
        ...
        if (this_catalog not in catalogs) and (catalog != 'all'): continue
...
def parse_glass_file(filename):
    ...
    for line in f:
        ...
        if line.startswith('NM '):
            ...
            glass_catalog[glassname]['dispform'] = int(float(nm[2]))  # int(float(nm[2]))
            glass_catalog[glassname]['nd'] = float(nm[4])
            glass_catalog[glassname]['vd'] = float(nm[5])
            glass_catalog[glassname]['exclude_sub'] = 0 if (len(nm) < 7) else int(float(nm[6]))  # int(float(nm[6]))
            glass_catalog[glassname]['status'] = 0 if (len(nm) < 8) else int(float(nm[7]))  # int(float(nm[7]))
            glass_catalog[glassname]['meltfreq'] = 0 if ((len(nm) < 9) or (nm.count('-') > 0)) else int(float(nm[8]))  # int(float(nm[8]))
...
if (__name__ == '__main__'):
    ...
    wavemax = 764.0
    nwaves = 3
    glasslib = ZemaxGlassLibrary(catalog = 'schott', wavemin = wavemin, wavemax = wavemax, nwaves = nwaves)
    # glasslib.plot_dispersion('N-BK7', 'schott')
    glasslib.plot_temperature_dependence('N-BK7', 'schott', arange(293, 350))
